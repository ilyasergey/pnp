...
Require Import MathComp.path Ssreflect.fintype ordtype.
...
Lemma zipC f1 f2 : zip f1 f2 = zip f2 f1.
Proof.
case: f1 f2=>s1 H1 [s2 H2] /=. 
move: {-}(erefl (zip' s1 s2)) (zip_sorted' H1) (zip_sorted' H2).
move: {-1}(zip' s1 s2)=>s; rewrite zipC' => <-{s}. 
case: (zip' s2 s1)=>// a qf1 qf2.
by congr (Some (FinMap _)); apply: proof_irrelevance.
Qed.

Lemma zipA f1 f2 f3 : 
        obind (zip f1) (zip f2 f3) = obind (zip^~ f3) (zip f1 f2).
Proof.
case: f1 f2 f3=>s1 H1 [s2 H2] [s3 H3] /=.
move: (zip_sorted' _) (zip_sorted' _). 
case E1: (zip' s2 s3)=>[w1|]; 
case E2: (zip' s1 s2)=>[w3|] //= qf1 qf2; last first.
- by move: (zipA' s1 s2 s3) (zip_sorted' _); rewrite E1 E2 /= => <-. 
- by move: (zipA' s1 s2 s3) (zip_sorted' _); rewrite E1 E2 /= => ->. 
move: (zipA' s1 s2 s3) (zip_sorted' _) (zip_sorted' _).
rewrite E1 E2 /= => ->; case: (zip' w3 s3)=>// s rf1 rf2.
by congr (Some (FinMap _)); apply: proof_irrelevance.
Qed.

Lemma zip_unitL f : zip (zip_unit f) f = Some f.
Proof.
case: f=>s H /=; move: (zip_sorted' _); rewrite zip_unitL' => pf.
by congr (Some (FinMap _)); apply: proof_irrelevance.
Qed.
