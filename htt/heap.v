Require Import Ssreflect.ssreflect Ssreflect.ssrfun Ssreflect.ssrnat. 
Require Import MathComp.div Ssreflect.ssrbool Ssreflect.seq. 
Require Import MathComp.path Ssreflect.eqtype.
...
Definition heapUMCMixin := 
  UMCMixin ftE tfE defE undefE emptyE updE domE dom_eqE freeE
           findE unionE umfiltE empbE ptsE.
...
Definition heap_pts A (x : ptr) (v : A) := 
  @UMC.pts heapUMC x (@idyn _ id _ v). 
...
Lemma heap_ind' (P : heap -> Prop) : 
         ...
         forall f, P f.
Proof. 
move=>H1 H2 H3; apply: gen_ind' H1 H2 _. 
...
rewrite /dom2 /dom' /fresh /supp -topredE /=. 
...
Proof. 
rewrite /dom2 /dom' /empb; case: h=>[|h] //=.
