...
programming and classical abstract algebra to interactive theorem
...
recursinon we have first considered Coq as a programming language in
\textbf{Chapter~\ref{ch:funprog}}. The progmming language intuition
helped us to move later to the realm of propositional logic and
...
universal and existential quantifiaction, though, required to appeal
...
defined in Coq and how is it used for proofs by rewriting. In the
...
minds. In \textbf{Chapter~\ref{ch:boolrefl}} we have seen how the same
...
decidable inductive predicates, has been supplied by a number of
examples in \textbf{Chapter~\ref{ch:ssrstyle}}, in which we have also
exercised in proofs by induction of different flavours. Mathematics is
a science of abstract strustures and facts, and formalizing such
structures and facts is an important of mathematical reasoning. In
...
I hope that this short and by all means not exchaustive course on
...
operate with a large vocabulary of facts, lemmas and tactis is a
...
a progressive resoning process and sense of achievement that few human
activities bring.

With this respect, it seems natural to advice to the reader to pick a
...
just to "have them in Coq at any price" is not a rewarding goal on its
own. What is less obvious is that setting up mathematical reasoning in
Coq usually brings some \emph{brand new} insights that usually come
...
of mastering the hunamity's programming expertise should pay back and
start being useful for the formal reasoning.
