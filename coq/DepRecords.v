...
We will be using partial commutative monoids (PCMs) as an
...
no interest with wildcards [_]. The [join] and [unit], therefore, bind
the operation and the identity element, whereas [Hc] and [Hlu] are
...
which are essentially orthogonal to those of a PCM. Moreover, at some
...
available from it, even if it's seen as being "wrapped" into the PCM
...
idea of the approach is to define a "wrapper" record type, which would
"pack" several mix-ins together, similar to how it is done in
object-oriented languages with implicit trait composition, e.g.,
...
record's members, rather than to rely on implicit mechanisms of field
linearization.}%
...
The dependent data structure declares two fields: the field [type] of
...
expose only the relevan, as abstract as possible, elements of the
...
of [U] affects the type of [x] and [y], therefore destructing it by
